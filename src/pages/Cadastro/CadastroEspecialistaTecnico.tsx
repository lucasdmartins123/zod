...
const esquemaCadastroEspecialista = z.object({
  ...
  especialidades: z.array(
    z.object({
      especialidade: z.string().min(1, "Preecha sua especialidade"),
      anoConclusao: z.coerce
        .number({
          errorMap: () => {
            return { message: "Insira um numero" };
          },
        })
        .min(1, "Preecha seu ano de conclusão"),
      instituicao: z.string().min(1, "Preecha sua instituição de ensino"),
    })
  ),
});
...
const CadastroEspecialistaTecnico = () => {
  ...
  const adicionavEspecialidade = () => {
    append({ especialidade: "", anoConclusao: 0, instituicao: "" });
  };
    ...
    <>
      ...
      <Form onSubmit={handleSubmit(aoSubmeter)}>
        ...
        <ButtonContainer>
          <Button
            type="button"
            onClick={adicionavEspecialidade}
            $variante="secundario"
          >
